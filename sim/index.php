...
<body>
  ...
  <div class="container py-4">
    ...
    <div class="comp-plan-card mb-4">
      ...
      <div class="comp-row">
        ...
        <div class="comp-body">
          ...
          <div class="comp-desc">
            ...
            <strong style="color: #e2e8f0">Breadth-First (BFS) placement</strong>
            — filling level by level, left before right.
          </div>
          <span class="comp-badge amber">Entry Fee − Goods Cost = Cash In Per Member</span>
        </div>
      </div>
      <div class="comp-row">
        ...
        <div class="comp-body">
          ...
          <div class="comp-desc">
            ...
            earns the
            <strong style="color: #e2e8f0">minimum of its two legs' accumulated volume</strong>, settled incrementally — so an unbalanced incoming volume
            ...
            <strong style="color: #fca5a5">flushed (permanently lost)</strong>. Inactive and capped members are skipped and earn nothing (their
            leg volume still accrues).
          </div>
          <span class="comp-badge amber">Matched volume min(L,R) · Daily cap = cap × pair volume ₱</span>
        </div>
      </div>
      <div class="comp-row">
        ...
        <div class="comp-body">
          ...
          <span class="comp-badge amber">Binary · Indirect · DFI — mirrored from packages</span>
        </div>
      </div>
      <div class="comp-row">
        <div class="comp-icon purple">
        ...
        <div class="comp-body">
          ...
          <div class="comp-desc">
            ...
            is their plan's
            <strong style="color: #c4b5fd">entry fee × lifetime cap multiplier</strong>. The cap covers
          ...
          <span class="comp-badge purple">Cap = Entry ₱ × Multiplier · Covers All Income Streams</span>
        </div>
      </div>
      <div class="comp-row">
      ...
      <div class="comp-row">
        ...
        <div class="comp-body">
          ...
          <span class="comp-badge green">Paid Once Per Direct Recruit · Blocked When Capped</span>
        </div>
      </div>
      <div class="comp-row">
      ...
      <div class="comp-row">
        ...
        <div class="comp-body">
          ...
          <span class="comp-badge red">Excess Volume = Lost Permanently · No Carry-Forward</span>
        </div>
      </div>
    </div>
    ...
    <div class="note-box mb-4">
      ...
      <strong style="color: #fca5a5">flushed permanently (no carry-forward)</strong>. The income cap is per-plan:
      ...
      <div class="row g-3">
          ...
          <div class="si">
            ...
            <label class="form-check form-switch plan-switch" id="sw-direct-wrap">
              <input
                ...
                id="sw-direct"
                checked />
              <span class="form-check-label">Pay direct referral on joins</span>
            </label>
            <div class="si-hint">
        ...
        <div class="col-12 col-sm-6 col-lg-3">
          <div class="si">
            ...
            <div class="auto-rule">
              <label class="auto-rule-opt"><input type="radio" name="automix-rule" value="equal" />
                Equal split</label>
              <label class="auto-rule-opt"><input type="radio" name="automix-rule" value="entry" />
                Inverse entry</label>
              <label class="auto-rule-opt"><input type="radio" name="automix-rule" value="bell" checked />
                Bell curve</label>
            </div>
            <div id="bell-sigma-wrap" style="display:none; margin-top:8px;">
              <div class="si">
                <div class="si-label">Spread (σ)</div>
                <div class="ctrl-row">
                  <input type="range" id="s-bellsigma" min="0.10" max="1.00" step="0.05" value="0.30" oninput="syncFromSlider('bellsigma')" />
                  <input type="number" class="num-input" id="n-bellsigma" value="0.30" min="0.05" max="2.00" step="0.05" oninput="syncFromNum('bellsigma')" />
                </div>
  ...
  <script>
    ...
    //  Default mirrors tmp/pckgs/pckgs_binary.json — the seeded live binary
    //  package lineup (Basic → Platinum, entry ₱5k–₱1M, binary/DFI toggles
    //  ON, indirect OFF, no unilevel, lifetime cap 3× entry, DFI 375 days).
    // ══════════════════════════════════════════
    const PRESETS = {
      default: {
        pcost: 30,
...
dfiDays: 90,
    ...
    function toggleBellSigma() {
      const bellEl = document.getElementById("bell-sigma-wrap");
      if (!bellEl) return;
      bellEl.style.display =
        autoMixEnabled() && autoMixRule() === "bell" ? "block" : "none";
    }
    document
    ...
    ["s-bellsigma", "n-bellsigma"].forEach((id) => {
      const el = document.getElementById(id);
      if (el)
        el.addEventListener("input", () => {
          const bellRadio = document.querySelector(
            'input[name="automix-rule"][value="bell"]',
          );
          if (autoMixEnabled() && bellRadio && !bellRadio.checked) {
            bellRadio.checked = true;
            toggleBellSigma();
          }
          if (autoMixEnabled() && autoMixRule() === "bell")
            applyAutoMixToDom();
        });
    });
    ...
    function autoMixRule() {
      ...
      return sel ? sel.value : "bell";
    }

    function bellCurveMix(plans, sigma, total) {
      total = Math.max(3, Math.round(total) || 2000);
      const indexed = plans.map((p, i) => ({
        entry: p.entry,
        idx: i
      }));
      indexed.sort((a, b) => a.entry - b.entry || a.idx - b.idx);

      const ranks = new Array(plans.length);
      let gi = 0;
      while (gi < indexed.length) {
        let gj = gi;
        while (
          gj < indexed.length &&
          indexed[gj].entry === indexed[gi].entry
        )
          gj++;
        for (let k = gi; k < gj; k++) {
          ranks[indexed[k].idx] = gi + (k - gi) / (gj - gi);
        }
        gi = gj;
      }

      const maxRank = Math.max(1, plans.length - 1);
      const weights = plans.map((_, i) => {
        const x = ranks[i] / maxRank;
        return Math.exp(-(x * x) / (2 * sigma * sigma));
      });
      const sumW = weights.reduce((a, b) => a + b, 0);
      if (plans.length < 2 || sumW <= 0) {
        return plans.map(() => 100 / Math.max(1, plans.length));
      }

      // Reserve exactly 1 account for the most expensive plan and 2 for the
      // second-most expensive (top tiers stay present but capped). Remaining
      // budget is spread across the lower tiers by their bell weights.
      const ordered = plans
        .map((_, i) => i)
        .sort((a, b) => ranks[b] - ranks[a]);
      const topIdx = ordered[0];
      const secondIdx = ordered[1];
      const guaranteed = new Set([topIdx, secondIdx]);

      const otherW =
        plans.reduce(
          (s, _, i) => s + (guaranteed.has(i) ? 0 : weights[i]),
          0
        ) || 1;
      const freeBudget = total - 3;
      const counts = plans.map((_, i) => {
        if (i === topIdx) return 1;
        if (i === secondIdx) return 2;
        return (weights[i] / otherW) * freeBudget;
      });
      return counts.map((c) => (c / total) * 100);
    }

    function autoMixPlans(plans) {
      const rule = autoMixRule();
      if (rule === "bell") {
        const sigma = getVal("bellsigma") || 0.3;
        const total = Math.round(getVal("maxm")) || 2000;
        return bellCurveMix(plans, sigma, total);
      }
      if (rule === "entry") {
    ...
    function addPlan() {
      ...
      setPlanValues({
        ...DEFAULT_PLAN,
        name: "Plan " + (count + 1)
      }, plansWrap.querySelectorAll(".plan-card")[count]);
      applyAutoMixToDom();
      updateAddBtn();
    }
    ...
    function simulate(p) {
      ...
      const log = [];
      let day = 0;
      const runSeq = buildRunSeq(plans, maxm);
      let runPos = 0;
      ...
      // Spread totalN members across plans proportionally to join mix, pacing
      // plan types evenly across the whole run so cumulative totals match the
      // mix exactly (tiny tiers are no longer lost to per-day rounding).
      function buildRunSeq(plansArr, totalN) {
        const totalW = plansArr.reduce((s, x) => s + x.mix, 0) || 1;
        const exact = plansArr.map((x) => (x.mix / totalW) * totalN);
        const counts = exact.map(Math.floor);
        let rem = totalN - counts.reduce((a, b) => a + b, 0);
        while (rem > 0) {
          let best = -1,
            bestFrac = -1;
      ...
      while (totalMembers < maxm) {
        ...
        const seq = [];
        for (let j = 0; j < toAdd; j++) seq.push(runSeq[runPos++]);
    ...
    <div class="flow-item">
      ...
      <div class="fc-head">
        ...
        <span class="fc-mix">${fmtMix(pl.mix)} mix</span>
      </div>
